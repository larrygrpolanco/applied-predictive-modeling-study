...
APM_URL = ('https://cran.r-project.org/src/contrib/Archive/'
            'AppliedPredictiveModeling/AppliedPredictiveModeling_1.1-6.tar.gz')
...
def download_pack(datasets_folder):
    ...
    opener = urlopen(APM_URL, context=ctx)
    ...
    opener = urlopen(CRT_URL, context=ctx)
    open(archive_path, 'wb').write(opener.read())
        ...
        for name in files:
            ...
            if name.endswith('.RData'):
                ...
                with localconverter(robjects.default_converter + pandas2ri.converter):
                    for var in robj:
                        myRData = robjects.globalenv[var]
